...
def get_args_parser():
    parser = argparse.ArgumentParser('ReViT training and evaluation script', add_help=False)
    ...
    parser.add_argument('--scratch', action='store_true', default=False,
                        help='use pretrained model')
    parser.add_argument('--EMB', default=768, type=int)
...
def main(args):
    ...
    if args.scratch:
        args.output_dir = args.output_dir+'/scratch_revit_EMB_'+str(args.EMB)+'_'+str(args.epochs)+'_lr_'+str(args.lr)+'_a_'+str(args.distillation_alpha)+'_T_'+str(args.distillation_tau)
    elif args.finetune:
    ...
    EMB = args.EMB
    head = [10,round(EMB*0.005)*2,round(EMB*0.005)*2,round(EMB*0.005)*4,round(EMB*0.005)*4,round(EMB*0.005)*4,round(EMB*0.005)*4,round(EMB*0.005)*4,round(EMB*0.005)*4,round(EMB*0.005)*2,round(EMB*0.005)*2,10]
    QK = [round(EMB*0.05/8)*16,round(EMB*0.05/8)*8,round(EMB*0.05/8)*8,round(EMB*0.05/8)*16,round(EMB*0.05/8)*16,round(EMB*0.05/8)*16,round(EMB*0.05/8)*16,round(EMB*0.05/8)*16,round(EMB*0.05/8)*16,round(EMB*0.05/8)*8,round(EMB*0.05/8)*8,round(EMB*0.05/8)*16]
    V = [64]*12
    MLP = [EMB*3,EMB*3,EMB*3,EMB*6,EMB*6,EMB*6,EMB*6,EMB*6,EMB*6,EMB*3,EMB*3,EMB*3]
    ...
    if args.distillation_type != 'none':
        CNN_teacher_model = teacher_model
